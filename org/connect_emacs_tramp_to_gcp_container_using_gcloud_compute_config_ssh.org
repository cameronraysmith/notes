...
\begin{equation*}
\partial_{T} \rho(n, T)=\underbrace{D \partial_{n}^{2}(n \rho(n, T))}_{\text {drift }}-\underbrace{s \partial_{n}(n \rho(n, T))}_{\text {selection }} - \underbrace{N \mu \partial_n \rho(n,T)}_{\text{mutation}}
\end{equation*}
...
Deploy a vm based on the container =cameronraysmith/notebooks:latest=: [[file:a_containerized_jupyter_notebook_with_multiple_kernels_on_Arch_Linux.org][a containerized jupyter notebook with multiple kernels on Arch Linux]] using [[https://cloud.google.com/compute/docs/containers/deploying-containers#deploying_a_container_on_a_new_vm_instance][deploying a container on a new vm instance]] and [[https://cloud.google.com/compute/docs/containers/configuring-options-to-run-containers][configuring options to run containers]].
...
With TRAMP, it would be best to have your [[file:google_cloud_platform.org][google cloud platform]] instances entered into your local ssh hosts automatically. This was included in a response [[https://emacs.stackexchange.com/a/50534/19156][spacemacs - tramp gcloud compute ssh not working - Emacs Stack Exchange]], but of course it is also available in the [[https://cloud.google.com/sdk/gcloud/reference/compute/config-ssh][gcp documentation for config-ssh]], which is what we have used above.

Here is a brief tutorial on TRAMP explaining how to [[https://willschenk.com/articles/2020/tramp_tricks/][connect to and run code in a docker container on a remote host]].

I installed the package [[https://github.com/emacs-pe/docker-tramp.el][docker-tramp.el]] which adds a docker method to TRAMP. TRAMP essentially allows manipulation of remote files by adding some metadata in front of a colon in front of the filename much like the =scp= interface. In this case one treats docker as an addition prefix in front of an additional colon. So, for example, you could refer to a file with:
...
You can tell if you're working inside a docker container by examining the contents of the =proc/self/cgroup= file as discussed in [[https://tuhrig.de/how-to-know-you-are-inside-a-docker-container/][How to know you are inside a Docker container]].

#+BEGIN_SRC sh :results output verbatim replace :exports both :dir /docker:quizzical_blackwell:
...
:ID:       1217db72-1d36-4411-b55f-3abc3e183306
:END:
...
#+RESULTS: test-maxima-01
file_search1: /ssh:notebooks-vm.us-central1-f.quarere|docker:klt-notebooks-vm-\
cjme:/tmp/maxima-YSJeHL.max not found in file_search_maxima.
...
:PROPERTIES:
:header-args: :results output verbatim replace :session jupyter-julia-01 :dir /ssh:notebooks-vm.us-central1-f.quarere|docker:klt-notebooks-vm-cjme:  :exports both  :eval never-export :async yes
:END:
